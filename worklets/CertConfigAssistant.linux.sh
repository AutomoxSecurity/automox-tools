...
cert_file="${cert_path}/${cert_name}"
# grab a list of non-system (user) profiles
...
if [ -f /etc/lsb-release ]; then
    root_store="/etc/ssl/certs/ca-certificates.crt"
elif [ -f /etc/redhat-release ]; then
    root_store="/etc/ssl/certs/ca-bundle.crt"
fi
if [ "$root_store" == "" ]; then
    echo "Unable to find a trusted root store for this instance, exiting." 1>&2
    exit 1
fi

echo "Generating root certificate dump"
cat "${root_store}" > "${tmp_path}"
if [ "${tmp_certs}" != "" ]; then
    echo "Appending worklet provided certificate to root bundle"
  ...
  for app_dir in "${java_app_dirs[@]}"; do
    ...
    if [ "$app_dir" != "" ] && [ "${app_dir:0:1}" == "." ]; then
        ...
        for user in ${users}; do
            ...
            user_home=$(grep -E "^${user}" /etc/passwd | awk -F: '{print $6}')
            user_root="${user_home}/${app_dir:2}"
